...
See `docs/deployment.md` → *RLS hardening* for the full pattern and tradeoffs.

## Concurrency notes

- **Single-use invites** don't rely on read-then-write. Acceptance performs an `UPDATE ... WHERE id = ? AND accepted_at_ms IS NULL AND revoked_at_ms IS NULL RETURNING id`; zero rows updated = someone got there first. Safe under concurrent clicks without explicit transactions — and Postgres serializes the conditional update correctly under load.
- **Seat counting**: `seatsUsed = COUNT(memberships)` per org. Enforced at invite-accept time (`assertSeatAvailable`); at single-org scale this is exact, and the claim+count could be wrapped in one transaction for stricter multi-request races (documented limit).
