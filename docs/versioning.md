...
1. **GitHub Releases page**: `https://github.com/verdantstack/sveltekit-postgres-starter/releases`
   - Each release has a tag (e.g. `v0.1.2`) and release notes from `CHANGELOG.md`.
2. **Clone a specific tag**:
   ```bash
   git clone --branch v0.1.2 https://github.com/verdantstack/sveltekit-postgres-starter.git
   ```
3. **Download a specific version**:
   ```bash
   curl -L https://github.com/verdantstack/sveltekit-postgres-starter/archive/refs/tags/v0.1.2.zip -o starter-v0.1.2.zip
   ```
...
|---------|------|---------|
| 0.1.0 | 2026-08-31 | Initial release — Postgres port of the multi-tenant starter: Drizzle Postgres schema + migrations, postgres.js driver with pooled connections, opt-in RLS policies, docker-compose local Postgres, 206 tests |
